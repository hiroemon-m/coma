...
class Env(nn.Module):
    def __init__(self, agent_num,edge, feature,alpha,beta,gamma,persona,coefficient_vector) -> None:
        #t=4での状態に初期化
        super().__init__()
        ...
        self.coefficient_vector = coefficient_vector.clone().detach().requires_grad_(False)
        
  

    def reset(self, edges, attributes,persona,coefficient_vector):
        self.edges = edges
    ...
    def step(self,next_feature,next_action,time):
        with torch.no_grad():
            ...
            persona_num = self.coefficient_vector[time].size()[1]
            #impactを計算:(L2ノルム)^2
            ...
            persona_gamma = torch.mm(self.coefficient_vector[time],self.gamma.view(persona_num,1)) #gammaを計算
            ...
            persona_alpha = torch.mm(self.coefficient_vector[time],self.alpha.view(self.coefficient_vector[time].size()[1],1))
            reward_sim = similality_coo.multiply(persona_alpha).coalesce()

            #costを計算
            persona_beta = torch.mm(self.coefficient_vector[time],self.beta.view(self.coefficient_vector[time].size()[1],1))
    ...
    def update_reward(self,next_feature,one_hop_action,time,scaler,alpha_all=None,beta_all=None,gamma_all=None):
        ...
        if time == 0:
            ...
            alpha_all = torch.matmul(self.coefficient_vector[time], self.alpha).squeeze()
            beta_all = torch.matmul(self.coefficient_vector[time], self.beta).squeeze()    
            gamma_all = torch.matmul(self.coefficient_vector[time], self.gamma).squeeze()
