...
#another file
from actor_review_4 import Actor
...
def execute_data(persona_num,data_name,data_type):
    ...
    path_save = "experiment_data/action_space/n_hop"
    np.save(path_save+"/train_persona_ration",np.concatenate([mixture_ratio.detach().numpy()],axis=0))
    np.save(path_save+"/train_paramerter",np.concatenate([T.detach().numpy(),e.detach().numpy(),r.detach().numpy(),w.detach().numpy(),x.detach().numpy(),s.detach().numpy()],axis=0))
