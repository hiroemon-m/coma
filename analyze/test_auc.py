...
def execute_data(persona_num,data_name,data_type):
    ...
    path = "../experiment_data/action_space/variable/space=10%/train_paramerter.npy"
    paramerter = np.load(path)
    paramerter = paramerter.astype("float32")
    print("paramerter",paramerter)
    ...
    agents = PPO(obs,agent_num, input_size, action_dim,lr, gamma,T,e,r,w,x,s,persona_ration,temperature,story_count,data_name)
    ...
    for count in range(10):
        ...
        for test_time in range(TOTAL_TIME - GENERATE_TIME):
            ...
            edge_action,edge_prob ,feat_prob ,feat_action = agents.actor.pred(feature,edges,two_hop_neighbar,0,test_time,action_dim,feat_size)
