...
- エッジ生成の探索範囲を変える
現状:エッジの探索範囲 = 2hopの未接続ノード + ランダムなノード
変更:エッジの探索範囲 = 3hopの未接続ノード + ランダムなノード 
"""
...
class Actor(nn.Module):
    ...
    def _create_n_hop_discconect(self,n_hop,one_hop):
        """n-hopの未接続ノードとのエッジ確率の計算."""
        indices_A = n_hop.indices()
        values_A = n_hop.values()
        size = n_hop.size()
        ...
        create_edge = remove_diagonal(create_edge)

        return create_edge

    def _create_n_hop_neighbar(self,one_hop,n):
        """n-hopの隣接行列の計算."""
        n_hop = one_hop
        for i in range(n):
            n_hop = torch.sparse.mm(n_hop,one_hop)
    ...
    def forward(self, attributes, edges, two_hop_neighbar, total_past, times, agent_num, sparse_size):
        ...
        for i in range(len(self.persona[0][0])):
            ...
            n_hop_neighbar = self._create_n_hop_neighbar(edges,hop_num)
            n_hop_edges = n_hop_neighbar.detach().clone()
            n_hop_disconnect_edges = self._create_n_hop_discconect(n_hop_edges,edges.coalesce())
            
            connect_edges_action_space = (n_hop_disconnect_edges + random_node).coalesce()
