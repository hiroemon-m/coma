...
def execute_data(trial):
    ...
    #パラメータ
    mu = trial.suggest_float("mu",0.01,0.9,log=True)
    lr =  trial.suggest_float("lr",1e-3,1e-2,log=True)
    def_t = trial.suggest_float("T",0.5,1.5,log=True)
    def_e = trial.suggest_float("e",0.5,1.5,log=True)
    def_r = trial.suggest_float("r",1e-3,1.0,log=True)
    def_w = trial.suggest_float("w",1e-3,1.0,log=True)
...
if __name__ == "__main__":
    ...
    for i in [5]:
        ...
        print(f"最適なパラメータが {output_file} に保存されました。")
